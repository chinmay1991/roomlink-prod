      ...
      <div className="flex-1 space-y-3 overflow-y-auto py-2">
        {initialMessages.map((m) => (
          <div key={m.message_id} className={cn('max-w-[80%]', m.sender_type === 'guest' ? 'ml-auto text-right' : '')}>
            ...
            <p className="mt-0.5 text-xs text-slate-400">{m.sender_type === 'guest' ? 'You' : 'Reception'} · {timeAgo(m.sent_at)}</p>
          </div>
        ))}
        {initialMessages.length === 0 && (
          <p className="pt-10 text-center text-sm text-slate-500">Send a message to start a conversation with Reception.</p>
        )}
        <div ref={bottomRef} />
      </div>
