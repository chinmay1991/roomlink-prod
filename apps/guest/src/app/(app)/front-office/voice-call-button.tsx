...
/**
 * "Call Front Office" — guest side of the ZegoCloud voice calling feature.
 * Rings every Front Office/admin staff member at this hotel at once (v1 has
 ...
 * Front Office" instead of on every load of this page.
 */
export function VoiceCallButton() {
  ...
  async function startCall() {
    ...
    if (!res.ok) {
      ...
      setError(data?.error ?? 'Unable to reach Front Office. Please try again.')
      setState('idle')
      return
    }
    ...
    if (call.calleeIds.length === 0) {
      setError('No one at Front Office is available right now. Please try again shortly.')
      endCall()
      return
    }
    ...
    zp.setCallInvitationConfig({
      ...
      onOutgoingCallRejected: () => {
        setError('Front Office declined the call.')
        endCall()
      },
    ...
    // Room number/guest name for Front Office's incoming-call popup — sent as
    ...
    async function attemptInvite() {
      return zp.sendCallInvitation({
        callees: call.calleeIds.map((id) => ({ userID: id, userName: 'Front Office' })),
    ...
    if (!succeeded) {
      ...
      setError('Unable to reach Front Office. Please try again.')
      endCall()
      return
    }
    ...
    <div className="space-y-2">
      ...
      {state === 'ringing' && (
        <Button variant="danger" onClick={endCall} className="w-full">
          ...
          Ringing Front Office… tap to cancel
        </Button>
      )}
      ...
      {state === 'connected' && (
        <Button variant="danger" onClick={endCall} className="w-full">
          <PhoneOff className="h-4 w-4" aria-hidden />
          On call with Front Office — tap to hang up
        </Button>
      )}
    </div>
  )
