 ...
 * STAFF_NAV_ITEMS/FRONT_OFFICE_NAV_SECTIONS rather than imported from a
 * shared module — see the mobile-app plan's "Isolation mechanism":
 * sidebar.tsx must never be touched by mobile-app work, so it can't be
 * refactored into a shared source of truth. Update both lists in tandem when
 * a role's pages change.
 */
...
const FRONT_OFFICE_ITEMS: MobileNavItem[] = [
...
export function mobileNavItemsForRole(roleName: string | null): MobileNavItem[] {
  ...
  if (isFrontOfficeRoleName(roleName)) return FRONT_OFFICE_ITEMS
