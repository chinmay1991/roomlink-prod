...
/** Reception PRD §17/§18 — thread + reply, plus a read-only cross-reference to the room's open requests. */
export default async function ConversationDetailPage({ params }: { params: { conversationId: string } }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Link href="/hotel/reception-desk/conversations" className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700">
        ...
        {messages.map((m) => (
          <div key={m.message_id} className={cn('max-w-[80%]', m.sender_type === 'staff' ? 'ml-auto text-right' : '')}>
            ...
            <p className="mt-0.5 text-xs text-slate-400">
              {m.sender_type === 'guest' ? 'Guest' : 'Reception'} · {timeAgo(m.sent_at)}
            </p>
          </div>
        ))}
