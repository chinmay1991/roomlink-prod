...
import { getFrontOfficeDashboard, getDepartmentMonitoring, getRoomOverview } from '@/server/services/front-office.service'
...
/** Front Office (formerly Reception PRD) §5/§21/§22 — hotel-wide operational dashboard, restyled to the requested mockup. Every panel reads real hotel data (requests, guest_sessions, conversations, alerts) already served by this app's Front Office module; nothing here is fabricated. "Reserved" rooms and pre-arrival/departure forecasts are intentionally omitted — there is no reservation/booking table anywhere in the schema to back them. */
export default async function FrontOfficeDashboardPage() {
  ...
  const [kpis, departments, rooms, openRequests, alerts, conversations, recentCalls] = await Promise.all([
    getFrontOfficeDashboard(actor.hotelId, actor),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">{greeting}, Front Office 👋</h1>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
        <StatTile label="Guests In House" value={kpis.guestsInHouse} sublabel="Live count" icon={Users} color="blue" href="/hotel/front-office-desk/rooms" />
        <StatTile
          label="New Requests"
        ...
        <div className="space-y-5 lg:col-span-3">
          ...
          <Card>
            <CardHeader className="flex items-center justify-between">
              ...
              <Link href="/hotel/front-office-desk/conversations" className="text-xs font-medium text-brand-600 hover:text-brand-700">
                View all
              </Link>
            </CardHeader>
            <ul className="divide-y divide-slate-100">
              {needsReply.map((c) => (
                <li key={c.conversation_id}>
                  <Link href={`/hotel/front-office-desk/conversations/${c.conversation_id}`} className="block px-5 py-3 text-sm hover:bg-slate-50">
          ...
          <Card>
            <CardHeader className="flex items-center justify-between">
              ...
              <Link href="/hotel/front-office-desk/call-logs" className="text-xs font-medium text-brand-600 hover:text-brand-700">
                View all
              </Link>
            </CardHeader>
            <ul className="divide-y divide-slate-100">
      ...
      <Card className="p-5">
        <div className="mb-4 flex items-center justify-between">
          ...
          <Link href="/hotel/front-office-desk/rooms" className="text-xs font-medium text-brand-600 hover:text-brand-700">
            View all rooms
          </Link>
        </div>
