...
import { createFrontOfficeStaffSchema, CreateFrontOfficeStaffInput } from '@/server/validation/staff.schema'

type FrontOfficeStaffRow = { user_id: string; full_name: string; email: string; employee_id: string | null; status: string }

export function FrontOfficePanel({ frontOfficeStaff }: { frontOfficeStaff: FrontOfficeStaffRow[] }) {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<CreateFrontOfficeStaffInput>({ resolver: zodResolver(createFrontOfficeStaffSchema) })

  async function onSubmit(values: CreateFrontOfficeStaffInput) {
    setSubmitting(true)
    const res = await fetch('/api/v1/hotel/front-office', {
      method: 'POST',
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {frontOfficeStaff.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-5 py-10 text-center text-sm text-slate-500">
                    No Front Office staff yet.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <Card>
        <CardHeader>
          <h2 className="text-sm font-semibold text-slate-900">Add Front Office staff</h2>
        </CardHeader>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              <Button type="submit" disabled={submitting}>
                {submitting ? 'Adding…' : 'Add Front Office staff'}
              </Button>
            </div>
          </form>
        </CardBody>
