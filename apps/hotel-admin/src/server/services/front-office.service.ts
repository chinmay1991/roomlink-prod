...
import { isFrontOfficeRoleName } from '@/server/services/hotel-roles.service'
 ...
 */
export function requireFrontOfficeOrAdmin(user: HotelSessionUser) {
  if (user.userType === 'hotel_admin') return
  if (isFrontOfficeRoleName(user.roleName)) return
  throw new ForbiddenError('Front Office access only')
}
...
/**
 * Front Office (formerly Reception PRD) §5 — hotel-wide KPI row. Same shape as `getManagerQueueKpis`,
 ...
 */
export async function getFrontOfficeDashboard(hotelId: string, actor: HotelSessionUser) {
  requireFrontOfficeOrAdmin(actor)
  const today = startOfToday()

  const [kpiRows, openWork, conversations] = await Promise.all([
    prisma.$queryRaw<FrontOfficeKpiRow[]>`
      SELECT
...
/**
 * Front Office (formerly Reception PRD) §22 — monitoring only, hotel-wide, one row per department.
 *
...
/** Front Office (formerly Reception PRD) §23 — limited operational view only (no confidential employee info). */
...
/** Front Office (formerly Reception PRD) §21 — room number/type/occupancy/active session/open requests. Not a PMS. */
...
/** Front Office (formerly Reception PRD) §19 — search by room number / guest name / stay id / request id. Minimal fields only. */
